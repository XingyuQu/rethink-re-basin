...
class Manager:
    ...
    def __init__(self, args):
        ...
        if args.mask_onnx:
            ...
            if args.eval_fast:
                ...
                logging.info("The masked acc and loss are ", init_acc_top1, init_loss)
    ...
    def run_policies_for_method(self, policy_type: str, method_name: str, **kwargs):
        ...
        for policy_obj in getattr(self, f"{policy_type}s"):
            # logging.info(f"policy obj in {policy_type} is ", policy_obj)
            # run the method_name'd method for every attribute of policy iobjects
            retval = getattr(policy_obj, method_name)(**kwargs)
            # logging.info("retval is ", retval)
            # if isinstance(retval, tuple):
            #     if isinstance(retval[0], torch.Tensor):
            #         retval = retval[0].item()
            res.append(retval)
        ...
        if policy_type == 'pruner' and method_name == 'on_epoch_begin':
            ...
            # logging.info('meta receive by run policy ', meta)
    ...
    def run(self):
        ...
        for epoch in range(self.initial_epoch, self.n_epochs):
            ...
            if 'ln' in self.args.arch or self.args.arch == 'my_cifar_vgg16':
            ...
            if self.args.arch in ['my_cifar_vgg16_bn', 'my_cifar_resnet20']:
            ...
            epoch_results_top1.append(test_acc_top1) # repair prune acc
            epoch_results_top5.append(test_acc_top5)  # repair prune accK
            ...
            for i, batch in enumerate(train_loader):
                ...
                def agg_func(x):
                    res = x[0]
                    if len(x) > 1:
                        for id_res in range(len(res)):
                            for id_x in range(1, len(x)):
                                res[id_res] += x[id_x][id_res]
                    return res
                loss, acc = self.run_policies_for_method('trainer',
                                                        ...
                                                        device=self.device,
                                                        loss=0.,
                                                        agg_func=agg_func) # lambda x: np.sum(x, axis=0)
