...
class ODLSTM(LSTM):
    def __init__(self, *args, od_layer: bool = True, **kwargs):
        ...
        if self.num_layers > 1:
            raise ValueError(
                "Do not use multiple lstm layers as one lstm cell,"
                "stack them instead.")
        self.full_hidden_size = self.hidden_size
        self.od_layer = True
        self.layer_width = self.full_hidden_size
    ...
    def forward(self, x, hx=None, p=None):
        ...
        ind_out_ext = np.concatenate(
            [np.array(ind_out) + i * self.full_hidden_size
             for i in range(4)], axis=0)
        self._flat_weights = []
        # subsampled weights and biases
        for _, p_name in enumerate(self._flat_weights_names):
