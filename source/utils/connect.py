...
class ReshiftLayer(nn.Module):
    def __init__(self, layer):
        super().__init__()
        self.layer = layer
        self.bn = BatchShift2d(layer.out_channels)
...
def reset_bn_stats(model, device, bn_loader):
...
def repair(loader, model_tracked_s, model_repaired, device, alpha_s=None,
    ...
    for model in model_tracked_s:
        reset_bn_stats(model, device, loader)
    ...
    reset_bn_stats(model_repaired, device, loader)
